...
const TieStrengthDefinitionForm: React.FC<TieStrengthDefinitionProps> = ({ dataset }) => {
  ...
  // Update local state when dataset definition changes externally
  useEffect(() => {
  ...
  }, [dataset.tie_strength_definition, dataset.id]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error('Error defining tie strength:', err);
      setLocalError('Failed to define tie strength. Please try again.');
    } finally {
      setIsSubmitting(false);
    }
  };
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <Select 
              id="source_column" 
              name="source_column" 
              value={definition.source_column || ''} 
              onChange={handleInputChange} 
              options={[{ value: '', label: 'Select...' }, ...columnOptions]} 
              required 
            />
          </div>
          <div>
            ...
            <Select 
              id="target_column" 
              name="target_column" 
              value={definition.target_column || ''} 
              onChange={handleInputChange} 
              options={[{ value: '', label: 'Select...' }, ...columnOptions]} 
              required 
            />
        ...
        <div>
          ...
          <Select 
            ...
            value={definition.calculation_method} 
            onChange={handleInputChange} 
        ...
        {definition.calculation_method === 'attribute_value' && (
          <div>
            ...
            <Select 
              id="weight_column" 
              name="weight_column" 
              value={definition.weight_column || ''} 
              onChange={handleInputChange} 
              options={[{ value: '', label: 'Select...' }, ...columnOptions]} 
              required 
            />
